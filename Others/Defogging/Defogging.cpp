...
IplImage *quw(IplImage *src, int block, double w)
{
	...
	//保存暗原色先验的图像
	//cvSaveImage("D:\\result\\darkchannel.png", dark_channel);
	...
	//cvSaveImage("D:\\result\\toushelv.png", toushelv);
	...
	//cvSaveImage("D:\\result\\defog.png", dst);
...
{
	//打开图像
	src = cvLoadImage("D:\\pictures\\test1.png", -1);
	//创造窗口
	cvNamedWindow("src", CV_WINDOW_AUTOSIZE);
	cvShowImage("src", src);
	cvNamedWindow("dst", CV_WINDOW_AUTOSIZE);
	cvCreateTrackbar(tbarname1, "dst", &block, 15, on_trackbar1);
	cvCreateTrackbar(tbarname2, "dst", &w1, 100, on_trackbar2);
	cvWaitKey(0);
	cvReleaseImage(&src);
	cvReleaseImage(&dst);
}
